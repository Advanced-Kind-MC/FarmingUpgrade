...
public final class FertiliseListener implements Listener {
    ...
    public void onFertilise(BlockFertilizeEvent e) {
        ...
        if (!this.plugin.getFertilisableCrops().containsKey(block.getType())) {
            return;
        }
        ...
        Collection<Block> fertilisedBlocks = FarmingUpgrade.findAdjacentMaterials(this.plugin.getFertilisableCrops().keySet(), block, 1);
        for (Block fertilisedBlock : fertilisedBlocks) {
            ...
            Consumer<BlockState> fertiliserFunction = this.plugin.getFertilisableCrops().get(fertilisedState.getType());
